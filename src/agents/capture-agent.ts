import { Decision, CaptureResult } from '../types.js';
...
export class CaptureAgent {
  ...
  async parseDecision(rawText: string): Promise<CaptureResult> {
  ...
  private parseWithRegex(text: string): CaptureResult {
  ...
  private async parseWithLLM(text: string): Promise<CaptureResult> {
    const prompt = `
...
Return only valid JSON.`;
  ...
  private calculateConfidence(decision: Partial<Decision>, originalText: string): number {
    let confidence = 0.3; // Base confidence

    if (decision.title && decision.title.length > 5) confidence += 0.2;
    if (decision.alternatives && decision.alternatives.length > 0) confidence += 0.2;
    if (decision.assumptions && decision.assumptions.length > 0) confidence += 0.2;
    if (originalText.length > 20) confidence += 0.1;
